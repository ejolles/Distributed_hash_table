...
#define IP_LENGTH 15
    ...
    while (1) {
        ...
        if (in_msg_len == 0) {
            ...
            // Send pairs
            char* msg = NULL;
            msg = calloc(MAX_MSG_SIZE, sizeof(char));
            if (msg == NULL) {
                kv_list_free(list);
                continue;
            }
            ...
            memcpy(msg, &size_to_send, sizeof(uint32_t));
            size_t current_size = sizeof(uint32_t);
            ...
            for (size_t i = 0; i < list->size; ++i) {
                ...
                if (current_size + key_len + value_len > MAX_MSG_SIZE) {
                    // send to client
                    // We substract 1 from the size so that \0 is not sent
                    // at the end of the message
                    sendto(socket, msg, current_size - 1, 0, (struct sockaddr *) &cli_addr, addr_len);
        ...
        // ---- It is a get request ----
            ...
            if (val == NULL) {
                sendto(socket, "", 1, 0, (struct sockaddr *) &cli_addr, addr_len);
            } else {
